...
const HotWheelsCollectorsHub = () => {
  ...
  return (
    <div className="hwc-widget">
      <div className="hwc-header">
        <div className="hwc-header-content">
          <h3>Track & Display Builder</h3>
          <span className="hwc-ai-tag">AI CONCEPT</span>
        </div>
        <p className="hwc-header-desc">
          Tell us what you want to build, and our AI will design it for you.
        </p>
      </div>

      <div className="hwc-content">
        <div className="hwc-mode-section">
          <label>What are you building?</label>
          <div className="hwc-mode-grid">
            {modes.map((mode) => (
              <button
                key={mode.id}
                className={`hwc-mode-card ${buildMode === mode.id ? 'active' : ''}`}
                onClick={() => setBuildMode(mode.id)}
              >
                <span className="hwc-mode-icon">{mode.icon}</span>
                <span className="hwc-mode-label">{mode.label}</span>
                <span className="hwc-mode-desc">{mode.desc}</span>
              </button>
            ))}
          </div>
        </div>

        <div className="hwc-complexity-section">
          <label>Complexity Level</label>
          <div className="hwc-complexity-slider">
            {['simple', 'medium', 'complex'].map((level) => (
              <button
                key={level}
                className={`hwc-complexity-btn ${complexity === level ? 'active' : ''}`}
        ...
        <button 
          className={`hwc-generate-btn ${isGenerating ? 'loading' : ''}`}
          onClick={handleGenerate}
          disabled={!buildMode || isGenerating}
        >
          {isGenerating ? (
            <>
              <span className="hwc-spinner"></span>
              Designing...
            </>
          ) : (
            'IGNITE CONCEPT'
        ...
        {suggestion && (
          <div className="hwc-suggestion-result">
            <div className="hwc-result-header">
              <h4>{suggestion.name}</h4>
            </div>
            <p className="hwc-result-desc">{suggestion.desc}</p>
            
            <div className="hwc-parts-list">
              <span className="hwc-parts-label">You'll need:</span>
              <ul>
            ...
            <div className="hwc-pro-tip">
              <span className="hwc-tip-badge">PRO TIP</span>
              <p>{suggestion.tip}</p>
            </div>
          </div>
        )}
